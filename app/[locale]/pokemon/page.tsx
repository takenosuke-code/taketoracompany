...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "metadata.pokemon" });
  return {
    title: t("title"),
...
export default async function PokemonPage({
  ...
  const formatCategoryName = (name: string): string => {
    ...
    if (name.toLowerCase() === "pokemon") return isJa ? "ポケモンカード" : "Pokémon Cards";
    return name
      .split(/[-_\s]/)
                    ...
                    <div className="space-y-5 sm:space-y-6">
                      <div className="border-b border-amber-900/30 pb-3">
                        ...
                        <p className="text-xs sm:text-sm text-[#F2E8DC]/50 mt-1 font-light">
                          ...
                          {isJa ? t("product.pieces") : categoryProducts.length === 1 ? "product" : "products"}{" "}
                          {t("product.available")}
                        </p>
                      </div>
                              ...
                              <div className="aspect-[4/3] w-full relative bg-gradient-to-br from-stone-800/30 to-stone-900/30 overflow-hidden">
                                {product.image_url || product.image ? (
                                  <Image
                                    ...
                                    alt={
                                      isJa
                                        ? `京都たけとら - ${product.name} ポケモンカード`
                                        : `${product.name} - Pokémon card at Taketora Kyoto`
                                    }
                                    fill
